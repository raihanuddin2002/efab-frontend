...
import { createProductBrowser, updateProductBrowser } from '@/app/supabase-browser-actions'
import { isProductType, ProductFormValue, ProductType, productSchema } from '@/app/(AdminPanel)/dashboard/products/types.product'
...
export default function EditProduct({ product }: { product: ProductType }) {
    ...
    const handleFormSubmit = async (values: ProductFormValue) => {
        ...
        try {
            ...
            const response = await updateProductBrowser(productData)
            ...
            toast.success(`Product updated successfully (${response.product_code})`)
        } catch (error) {
            console.error(error)
    ...
    return (
        <div className='max-w-[800px] w-full mx-auto py-10'>
            <div className='card w-full bg-slate-50 px-4 py-10'>
                <h2 className='text-2xl font-bold mb-10 uppercase text-center'>Edit Product</h2>
